...
Server.prototype.newSocket = function(socket){
  let myproxy, rfd, needProxy = false, cacheType;
  socket.on('data', (buf)=>{
    ...
    if (REQUEST_FIRST_LINE_PATTERN.test(line)) { // It's a new request.
      ...
      let head = new header(buf.toString('ascii', 0, bodyIdx));
      ...
      switch (cacheType.type) {
        ...
        case 1: // JRScript substitution
          ...
          this.execJrs(cacheType, socket, head, rfd);
        break;
...
Server.prototype.readSubs = function(ct, socket) {
  ...
  try {
    ...
    let head = html.packHead(200, data.length, html.checkMime(file));
    socket.write(head);
    socket.write(data);
    socket.end();
  } catch (err) {
