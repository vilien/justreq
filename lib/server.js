...
class Server{
  ...
  newSocket(req, res) {
    ...
    res.sendHead = (statusCode, headers) => res.writeHead(statusCode,
      Object.assign({'Server': 'JR Server/' + this.opt.version}, headers, attachedHead));
  ...
  startPipe(req, res, chunk) {
    ...
    if (needProxy) {
      ...
      req.url = rewrite(req, this.opt.rules || []);
