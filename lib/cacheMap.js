...
const CacheMap = function(){
  this.fastMapFile = cwd + '/.jr/fast-map.json';
  this.slowMapFile = cwd + '/.jr/slow-map.json';
  try {
    this.fastMap = require(this.fastMapFile);
  } catch (err) {
    this.fastMap = {};
  }
  try {
    this.slowMap = require(this.slowMapFile);
  } catch (err) {
    this.slowMap = {};
  }
};

CacheMap.prototype.init = function(rules) {
  let pJrs = /\.jrs$/;
  this.rule = rules.map((item)=>{
    if (item.href instanceof RegExp) {
      ...
      try {
        ...
        xecho('Invalid rule: href = ' + item.href, 'error');
      }
    }
    return item;
  });
 ...
 * @param  {object} reducedData Reduced request body, without header.
 */
CacheMap.prototype.check = function(method, sUrl, reducedData){
 ...
 */
CacheMap.prototype.updateMap = function(key, markSlowmap) {
  let time = +new Date();
 ...
 */
CacheMap.prototype.match = function(sUrl) {
  let rule = {notfound:true};
  this.rule.map((item)=>{
    if (sUrl.match(item.patt)) {
 ...
 */
CacheMap.prototype.touchSlowMap = function(ignore, method, sUrl, reducedData, key) {
